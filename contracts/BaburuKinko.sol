...
contract BaburuKinko {
    ...
    function getBorrowerOrders(address borrower) external view returns (uint256[] memory activeOrderIds) {
        ...
        for (uint256 i = 0; i < storedOrderIds.length; i++) {
            if (orders[storedOrderIds[i]].borrower != address(0)) {
                activeOrderIds[writeIndex] = storedOrderIds[i];
                writeIndex++;
            }
        }
    }
    ...
    function _closeOrder(uint256 orderId, Order memory order, uint256 penaltyAmount) internal {
        ...
        if (penaltyAmount > 0) {
            _safeTransferFrom(msg.sender, DEAD_ADDRESS, penaltyAmount);
        }
        _safeTransfer(msg.sender, order.collateralAmount);

        emit Repaid(orderId, msg.sender, order.collateralAmount, order.borrowedBnb, penaltyAmount);
    }
